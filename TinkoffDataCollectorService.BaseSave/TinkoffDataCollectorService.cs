...
using SaveDataSaveService = SaveService.Data.SaveData;
        ...
        public async Task Run(CancellationToken cancellationToken)
        {
            try
            {
                foreach (var stock in await _tinkoffDataService.GetAllStoks(cancellationToken))
                {
                    ...
                    {
                        _logger.LogError(e, $"Programm.TinkoffDataServiceException:ErrorLoadCandles; {e.Message}");
                        continue;
                    }

                    try
                    {
                        await _saveService.Save(new SaveDataSaveService(stock.Name, candles), cancellationToken);
                    }
                    catch (SaveServiceException e)
                    {
                        _logger.LogError(e, $"Programm.SaveServiceException; {e.Message}");
                        continue;
                    }
                }
            }
            ...
            {
                _logger.LogError(e, $"Programm.TinkoffDataServiceException:ErrorLoadStoks; {e.Message}");
                throw new TinkoffDataCollectorServiceException(e.Message, e);
            }
            catch (OperationCanceledException e)
            {
                _logger.LogError(e, "Programm.OperationCanceledException");
                throw;
            }
        }
    }
